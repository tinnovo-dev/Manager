	...
	<xsl:template match="actions">
		<xsl:for-each select="./action">
			...
			<ul>
				...
				<li><a href="?management&#x26;module=news&#x26;table={@table}&#x26;action=3">
						<xsl:value-of
	...
	<xsl:template match="table">
		...
		<xsl:variable name="id">
			<xsl:choose>
				<xsl:when test="$action=1">
					<xsl:value-of select="php:function('getNewId4Table',$table)" />
				</xsl:when>
		...
		<div class="form">
			<form method="post" action="?management&#x26;action=5&#x26;subaction={$action}">
				<input type="hidden" name="support" value="{$module}:{$table}" />
				<xsl:for-each select="./field">
					<xsl:choose>
						...
						<xsl:when test="@type=0 or @type=1">
							...
							<p>
								<label for="{@name}" class="left"><xsl:value-of select="php:function('getMessage',string(@name))" />: </label>
								<input value="{$value}" type="text" name="{@name}" id="{@name}" class="field" tabindex="1" />
							</p>
						</xsl:when>
						<!-- textarea field -->
...
<!--								<select name="{@name}">								-->
									<xsl:value-of disable-output-escaping="yes" select="php:function('getControl2ForeignData',string(@foreign),string(@foreign_fields),boolean(@is_category))" />
<!--								</select> -->
							</p>
						</xsl:when>
					</xsl:choose>
